...
class ClaudeParser:
    ...
    def _message_db_handler(self, user_id, message: str) -> list:
        ...
        if user not in self.messages_db:
            with open(settings.PROMPT_DIR, 'r') as f:
                self.messages_db[user] = str(f"{HUMAN_PROMPT} {f.read()} {AI_PROMPT}")
        self.messages_db[user] += str(f"{HUMAN_PROMPT} {message} {AI_PROMPT}")
    ...
    def get_response(self, user_id, message):
        ...
        try:
            ...
            answer = response
            self.messages_db[user_key] += answer
            tokens = self.client.count_tokens(self.messages_db[user_key])
    ...
    def get_request(self, prompt, temperature):
